...
setup_genie ()
{
  ...
  if [ -z "$GVERSXSEC"    ]; then export GVERSXSEC="v3_00_04a"             ; fi
  ...
  setup genie_xsec   ${GVERSXSEC} -q ${XSECQUAL}
  if [ $? -ne 0 ]; then
      echo "$b0: looking for genie_xec ${GVERSXSEC}a -q ${XSECQUAL}"
